...
- [x] 全局搜索「吴彩」「吹彩」，统一为正确公司名（已确认无错误，代码中均为「吹彩」）
- [x] 移动端适配：订单列表页响应式优化（移动端卡片式布局，桌面端保持原有表格）
- [x] 移动端适配：导航栏折叠为汉堡菜单（Sheet 抽屉），统计卡片 2 列布局
- [x] 移动端适配：搜索樏 Tab 横向滚动，新建订单按钒常显示

## 新功能（第六十八批）

- [x] 复制订单功能：订单列表操作列「复制」按钮（已存在，后端 orders.duplicate + 前端 Copy 图标按钮）

## Bug修复（第六十九批）

- [x] Bug1：编辑订单时图片无法上传（复制/拖拽/点击文件均不可用）——修复 MinIO 公共 URL，新增 MINIO_PUBLIC_ENDPOINT 环境变量
- [x] Bug2：生成单据弹窗中国内外客户合同选项未正确禁用（国外客户应禁用国内采购合同Tab）——代码已正确实现，更新到 NAS 后生效
- [x] Bug3：生成合同报错，无法导出国内合同/PI/CI——增加 Puppeteer 60s 超时和详细错误日志，更新后可通过 docker logs 查看具体错误
- [x] Bug4：管理员查看业务员订单时，生成单据弹窗内业务员填写的信息为空——新增 documentDrafts 数据库表，单据草稿改为服务器存储，跨设备共享

## 新功能 + 上线优化（第七十批）

- [x] 图片上传进度条：ImageUploader 增加可视化进度条，显示上传实时状态
- [x] 草稿作者提示：管理员查看业务员单据草稿时显示「草稿由XXX于XX时间保存」
- [x] 草稿自动保存提示：自动保存后显示「草稿已自动保存」toast 提示
...
- [x] Bug：MinIO Access Key 错误——storage.ts 增加详细错误日志（显示凭据来源、endpoint、bucket），方便 NAS 调试
- [x] 产品明细型号显示缩短：只显示型号编码（如 "2519"），不再拼接型号名称
- [x] 产品明细表格去掉横向滚动：table-fixed + 百分比列宽 + 缩小 padding/字号，总金额直接可见
- [x] PI/CI 产品明细表格同步优化：同样 table-fixed、紧凑间距、无横向滚动
...
- [x] 修复错别字：DocumentDialog 中"吴彩"改为"吟彩"
- [x] 亚马逊合同自动预填供货商信息：打开弹窗时从客户档案读取税号、銀行账号等预填到乙方字段
- [x] 搜索关键词高亮：订单列表中匹配的客户名、订单描述、订单号高亮显示
- [x] 订单列表亚马逊图标：渠道标签旁加醒目小图标区分亚马逊采购单

## Bug 修复（第七十八批）

- [x] 修复 Excel 导出：国外客户+渠道标注时行4中间列改为空白，不再重复显示行3已有的日期
- [x] 修复 PDF 合同/PI/CI：LOGO 图片改为 Base64 内嵌，不依赖外部 CDN，解决 Docker 环境无法加载图片的问题

## 新功能（第七十九批）

- [x] 亚马逊合同默认供货方：恩平市亿丰塑料模具有限公司（含完整信息），支持切换其他供货商
...
- [x] 优化：吟彩对亿丰采购合同甲方信息栏增加税号（91440300MAD58M3244），与右边亿丰格式保持一致

## 优化（第九十六批）

- [ ] 优化：订单号自动生成（ODYC-YYYYMMDD-001 格式，唯一递增），新建订单时自动填入，并优化 UI 展示

## 新功能（第九十六批）

- [x] 前端 OrderForm.tsx：新建订单时自动调用 generateOrderNo 接口，自动填充订单号
- [x] 前端 OrderForm.tsx：订单号字段改为只读显示（带刷新按钮），编辑订单时保持可读
- [x] 前端 OrderForm.tsx：优化订单号 UI（显示自动生成标记、刷新按钮）
